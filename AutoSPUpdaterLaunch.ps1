Start from Function Start-RemoteUpdate  
...
#region Info 
<#
https://raw.githubusercontent.com/brianlala/AutoSPUpdater/master/AutoSPUpdaterLaunch.ps1
.SYNOPSIS
...
#Region Check for Azure fileshare 
$bits = Get-Item $launchPath | Split-Path -Parent
# Check if we are running this from an Azure File Share. 
# Anyhow this doesn't really work for some reason.
...
#EndRegion Check for Azure fileshare 
...
If (Confirm-LocalSession) # $true, Yes, local session 
{
...
#EndRegion Check patchPath 

#region Check on updates found 
...
#endRegion Check on updates found 
#endRegion Set Up Paths & Environment
...
#region Set HashTable RolesPerServer  
...
#region Warn for non UNC path and Server.Count more than
...
#endregion Warn for non UNC path and Server.Count more than

#region Check if local and Farm more than 1 server 
if ((Confirm-LocalSession) -and $farmServers.Count -gt 1) 
# Only do this stuff on the first (local) server, and only if we have other servers in the farm.
{
    ...
    {
        if ($password) 
        # In case this is an automatic re-launch of the local script, 
        # re-use the password from the remote auth credential
        {
...
#endregion Check if local and darm more than 1 server 
...
#region Pause Search 
# Only need to pause the Search Service Application(s) 
# if running SharePoint 2013 and only attempt on the first (local) server in the farm
if ($spVer -eq 15 -and (Confirm-LocalSession))
{
    Request-SPSearchServiceApplicationStatus -desiredStatus Paused @verboseParameter
}
#endregion Pause Search Service Application
...
# In case we are running this from a non-SharePoint farm server, 
# only do these steps for farm member servers
if ($farmservers | Where-Object {$_ -match $env:COMPUTERNAME}) 
# Had to do it this way for PowerShell backward compatibility
{
    try
    {
        # We only want to Install-Remote if we aren't 
        # already *in* a remote session, 
...
else # If the local server isn't a SharePoint farm server, 
# just attempt remote installs
{
...
#endregion Install Remote Patch Binaries
...
#region Launch CA-FarmServers.aspx 
if (Confirm-LocalSession)
{
...
if (Test-UpgradeRequired -eq $true)
{
    ...
    # If we've attempted 2 times and we're still 
    # getting an error with PSConfig, launch the GUI
...
#region Launch CA-PatchStatus.aspx
if (Confirm-LocalSession)
{
    $caWebApp = Get-SPWebApplication -IncludeCentralAdministration | Where-Object {$_.IsAdministrationWebApplication}
    if ($null -ne $caWebApp)
